...
{
  __line = line_number;
  codeptr_t cursor = lines_get_contents( __line );
...
do_next(basic_type* rv)
{
  ...
  //printf("NEXT CONTINUE %d p=%d\n", (int)f->line, (int)__stack_p);
  __line = f->line;
...
{
  token save_sym = sym;
  codeptr_t save_pointer = tokenizer_char_pointer(NULL);
  bool rv = false;
